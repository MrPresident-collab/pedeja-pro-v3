...
export async function createProductionParcel(input:ProductionParcelInput):Promise<Order>{
  ...
  if(uploadError){ await sb.rpc('cancel_customer_enviar_shipment',{p_shipment_id:shipmentId,p_reason:'Falha ao guardar a fotografia da encomenda'}).catch(()=>undefined); throw new Error('Não foi possível guardar a fotografia da encomenda. Tenta novamente.'); }
  const {error:attachError}=await sb.rpc('attach_enviar_package_photo',{p_shipment_id:shipmentId,p_storage_path:path});
  if(attachError){ await sb.storage.from('delivery-proofs').remove([path]).catch(()=>undefined); await sb.rpc('cancel_customer_enviar_shipment',{p_shipment_id:shipmentId,p_reason:'Falha ao associar a fotografia da encomenda'}).catch(()=>undefined); throw new Error('Não foi possível finalizar a prova da encomenda. Tenta novamente.'); }
  const now=new Date().toISOString();
  const parcel:any={pickup:{...input.pickup},destination:{...input.destination},recipient:{...input.recipient},package:{...input.package,photo:{ref:path}},vehicle:{type:input.vehicleType,configurationId:config.configurationId,configurationLabel:config.label},deliveryNotes:input.deliveryNotes,estimate:input.estimate,paymentMethod:input.paymentMethod,status:'a_procurar_estafeta',timeline:[{status:'a_procurar_estafeta',label:'Envio criado',at:now}],createdAt:now,updatedAt:now,shipmentId};
