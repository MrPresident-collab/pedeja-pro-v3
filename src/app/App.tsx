...
import TrackingScreen from '../features/customer/tracking/TrackingScreen';
...
import '../features/customer/tracking/tracking.css';

const SPLASH_MS = 1400;
type CustomerView = 'home' | 'comida' | 'compras' | 'lojas' | 'enviar' | 'tracking';
...
export default function App() {
  ...
  return <Customer tab={tab} onTabChange={setTab} address={address || 'Casa'} trackingShipmentId={trackingShipmentId} onOpenTracking={setTrackingShipmentId} />;
}

function Splash({ returningSession }: { returningSession: boolean }) { return <main className="splash" aria-label="Pedejá"><div className="splash-center"><div><div className="splash-brand">Pedejá<span className="brand-purple-dot">.</span></div><p>A promessa que se move</p>{returningSession && <small>A preparar a tua sessão…</small>}</div></div><div className="splash-next" aria-hidden="true">A carregar…</div></main>; }
function Welcome({ onEnter, onGuest }: { onEnter: () => void; onGuest: () => void }) { return <main className="welcome"><div className="brand-lockup"><strong>Pedejá<span className="brand-purple-dot">.</span></strong><small>A promessa que se move</small></div><div className="welcome-copy"><span>ENTRA NO ECOSSISTEMA</span><h1>Tudo o que precisas,<br />a caminho de ti.</h1><p>Comida, compras, lojas e envios — numa só experiência.</p></div><div className="welcome-actions"><button className="primary" onClick={onEnter}>Entrar</button><button className="secondary" onClick={onEnter}>Criar conta</button><button className="text-button" onClick={onGuest}>Continuar como convidado</button></div></main>; }
function Auth({ onBack, onSuccess }: { onBack: () => void; onSuccess: () => void }) { const [phone, setPhone] = useState('+244 '); const [otp, setOtp] = useState(''); const [step, setStep] = useState<'phone' | 'otp'>('phone'); const [busy, setBusy] = useState(false); const [notice, setNotice] = useState(''); async function sendCode() { setBusy(true); setNotice(''); const { error } = await supabase.auth.signInWithOtp({ phone: phone.replace(/\s+/g, '') }); setBusy(false); if (error) return setNotice(error.message); setStep('otp'); setNotice('Enviámos um código para o teu número.'); } async function verifyCode() { setBusy(true); setNotice(''); const { data, error } = await supabase.auth.verifyOtp({ phone: phone.replace(/\s+/g, ''), token: otp.trim(), type: 'sms' }); setBusy(false); if (error || !data.user) return setNotice(error?.message ?? 'Código inválido.'); onSuccess(); } return <main className="auth-page"><button className="icon-button" onClick={step === 'otp' ? () => setStep('phone') : onBack} aria-label="Voltar">←</button><div className="auth-copy"><span>PEDEJÁ</span><h1>{step === 'phone' ? 'Entra na tua conta.' : 'Confirma o teu número.'}</h1><p>{step === 'phone' ? 'Usa o teu número de Angola para continuar.' : `Código enviado para ${phone}`}</p></div>{step === 'phone' ? <input autoFocus value={phone} onChange={event => setPhone(event.target.value)} placeholder="+244 9xx xxx xxx" inputMode="tel" /> : <input autoFocus value={otp} onChange={event => setOtp(event.target.value)} placeholder="Código de 6 dígitos" inputMode="numeric" maxLength={6} />}{notice && <div className="notice">{notice}</div>}<button className="primary" disabled={busy} onClick={step === 'phone' ? sendCode : verifyCode}>{busy ? 'A processar…' : step === 'phone' ? 'Enviar código' : 'Confirmar'}</button></main>; }
function Address({ address, onChange, onContinue }: { address: string; onChange: (value: string) => void; onContinue: () => void }) { return <main className="address-page"><div className="address-icon"><MapPin /></div><span>ONDE ENTREGAMOS?</span><h1>Primeiro, diz-nos onde estás.</h1><p>A tua casa será o endereço principal. Podes adicionar outros depois.</p><label>Casa<input value={address} onChange={event => onChange(event.target.value)} placeholder="Ex.: Talatona, Luanda" /></label><button className="primary" onClick={onContinue}>Continuar</button><button className="text-button" onClick={onContinue}>Definir mais tarde</button></main>; }

function Customer({ tab, onTabChange, address, trackingShipmentId, onOpenTracking }: { tab: CustomerTab; onTabChange: (tab: CustomerTab) => void; address: string; trackingShipmentId: string | null; onOpenTracking: (shipmentId: string | null) => void }) {
  const [view, setView] = useState<CustomerView>(trackingShipmentId ? 'tracking' : 'home');
  const goHome = () => setView('home');
  const selectSection = (section: CustomerSection) => { if (section === 'comida') setView('comida'); if (section === 'compras') setView('compras'); if (section === 'lojas') setView('lojas'); if (section === 'enviar') setView('enviar'); };
  useEffect(() => { if (trackingShipmentId) setView('tracking'); }, [trackingShipmentId]);
  if (view === 'tracking' && trackingShipmentId) return <div className="app-shell"><TrackingScreen shipmentId={trackingShipmentId} onBack={() => { onOpenTracking(null); setView('home'); }} /></div>;
