...
function Auth({ onBack, onSuccess }: { onBack: () => void; onSuccess: () => void }) {
  const [phone, setPhone] = useState('+244 '); const [otp, setOtp] = useState(''); const [step, setStep] = useState<'phone' | 'otp'>('phone'); const [busy, setBusy] = useState(false); const [notice, setNotice] = useState('');
...
function Customer({ tab, onTabChange, address }: { tab: CustomerTab; onTabChange: (tab: CustomerTab) => void; address: string }) {
  ...
  const changeTab = (next: CustomerTab) => { onTabChange(next); setView('home'); };
  ...
  if (view === 'comida') return <div className="app-shell"><ComidaScreen onBack={goHome} address={address} /><BottomNav tab={tab} onChange={changeTab} /></div>;
  ...
  if (view === 'enviar') return <div className="app-shell"><EnviarScreen onBack={goHome} /><BottomNav tab={tab} onChange={changeTab} /></div>;

  if (tab === 'descobrir') return <div className="app-shell"><DiscoverScreen /><BottomNav tab={tab} onChange={changeTab} /></div>;
  if (tab === 'pedidos') return <div className="app-shell"><OrdersScreen /><BottomNav tab={tab} onChange={changeTab} /></div>;
  if (tab === 'perfil') return <div className="app-shell"><ProfileScreen onSignOut={() => { void supabase.auth.signOut(); setView('home'); onTabChange('inicio'); }} /><BottomNav tab={tab} onChange={changeTab} /></div>;

  return <div className="app-shell"><HomeScreen address={address} onSection={selectSection} /><BottomNav tab={tab} onChange={changeTab} /></div>;
}
...
function HomeScreen({ address, onSection }: { address: string; onSection: (section: CustomerSection) => void }) {
  ...
  const [businesses, setBusinesses] = useState<Business[]>([]); const [loading, setLoading] = useState(true);
  ...
  const experiences: Array<{ id: CustomerSection; label: string; image: string; Icon: typeof UtensilsCrossed }> = [
    { id: 'comida', label: 'Comida', image: '/comida.png', Icon: UtensilsCrossed }, { id: 'compras', label: 'Compras', image: '/compras.png', Icon: ShoppingBag }, { id: 'enviar', label: 'Enviar', image: '/enviar.png', Icon: Package }, { id: 'lojas', label: 'Lojas', image: '/lojas.png', Icon: Store },
  ];
