...
function HomeScreen({ address, profileName, onSection }: { address: string; profileName: string; onSection: (section: CustomerSection) => void }) {
  ...
  return <main className="screen home-screen"><header className="topbar"><div className="home-avatar" aria-label="Perfil"><UserRound size={19} /></div><button className="home-address"><MapPin size={15} />{address || 'Selecionar morada'}<ChevronDown size={16} /></button></header><section className="home-heading"><p>{profileName ? `Olá, ${profileName} 👋` : 'Olá 👋'}</p><h1>O que precisas hoje?</h1></section><div className="search"><Search size={19} /><span>Pesquisar comida, lojas ou produtos</span></div><section className="experience-grid" aria-label="Experiências Pedejá">{experiences.map(({ id, label, image, Icon }) => <button className="experience-card" key={id} onClick={() => { setSection(id); onSection(id); }}><img src={image} alt="" onError={event => { event.currentTarget.style.display = 'none'; }} /><span className="experience-fallback"><Icon size={28} /></span><strong>{label}</strong></button>)}</section><div className="section-row"><h2>{section === 'home' ? 'Perto de ti' : experiences.find(item => item.id === section)?.label}</h2></div><section className="business-list">{loading ? <div className="home-loading">A carregar opções perto de ti…</div> : businesses.length === 0 ? <div className="home-empty">Ainda não há parceiros disponíveis na tua zona.</div> : businesses.slice(0, 8).map(business => <button className="business-row" key={business.id} onClick={() => { const category = business.marketplace_category?.toLowerCase() ?? ''; onSection(category === 'comida' ? 'comida' : category === 'lojas' ? 'lojas' : 'compras'); }}><div className="business-image"><Store size={22} /></div><div><strong>{business.name}</strong><p>{business.description || 'Parceiro Pedejá'}</p><small><MapPin size={12} /> Disponível na tua zona</small></div><ChevronRight size={18} /></button>)}</section></main>;
}
