...
import {
  ...
  Package, Search, ShoppingBag, Store, UserRound, Utensils, ShieldCheck,
} from 'lucide-react';
...
type Business = { id: string; name: string; description: string | null; marketplace_category: string | null; status: string };
type Product = { id: string; business_id: string; name: string; description: string | null; image_url: string | null; price: number; currency_code: string };
...
function App() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [{ data: profile }, { data: businessesData }] = await Promise.all([
        profileQuery,
        supabase.from('businesses').select('id,name,description,marketplace_category,status').eq('status', 'active').order('name').limit(24),
      ]);
      if (!active) return;
  ...
  const openCategory = (next: Category) => { setCategory(next); if (next === 'send') setTab('home'); };
  const nav = (next: Tab) => { setCategory(null); setTab(next); };

  if (category) return <AppShell tab={tab} onNavigate={nav}><CategoryView category={category} businesses={filteredBusinesses} products={products} loading={loading} onBack={() => setCategory(null)} onLoadProducts={async (businessId) => { const { data } = await supabase.from('products').select('id,business_id,name,description,image_url,price,currency_code').eq('business_id', businessId).eq('status', 'active').order('sort_order'); setProducts((data as Product[] | null) ?? []); }} /></AppShell>;
  ...
  return <AppShell tab={tab} onNavigate={nav}>
    ...
    {tab === 'profile' && <ProfileView name={userName} authenticated={authenticated} onSignOut={async () => { setGuest(false); await auth?.signOut(); }} />}
  </AppShell>;
}
...
function HomeView({ name, businesses, loading, onCategory }: { name: string; businesses: Business[]; loading: boolean; onCategory: (category: Category) => void }) {
  return <div className="screen home-screen"><header className="topbar"><div className="location-button"><MapPin size={17} /><div><span>Entregar em</span><strong>Casa</strong></div><ChevronRight size={16} /></div><button className="icon-button" aria-label="Notificações"><Bell size={20} /></button></header><section className="hero-copy"><p className="eyebrow">{name ? `Olá, ${name}` : 'Olá 👋'}</p><h1>O que precisas<br /><em>hoje?</em></h1><div className="search-bar"><Search size={19} /><span>Procurar comida, lojas ou produtos</span></div></section><section className="category-grid"><CategoryCard category="food" onClick={() => onCategory('food')} /><CategoryCard category="shopping" onClick={() => onCategory('shopping')} /><CategoryCard category="send" onClick={() => onCategory('send')} /><CategoryCard category="stores" onClick={() => onCategory('stores')} /></section><section className="quick-strip">{['Perto de ti', 'Mais pedidos', 'Promo', 'Aberto', '<20 min'].map((label, index) => <button key={label} className={index === 0 ? 'selected' : ''}>{label}</button>)}</section><section className="section-block"><SectionHeading title="Perto de ti" action="Ver tudo" />{loading ? <LoadingRows /> : businesses.length === 0 ? <EmptyState title="Estamos a preparar a rede" text="Os negócios disponíveis aparecerão aqui assim que estiverem activos." /> : <BusinessRail businesses={businesses.slice(0, 8)} />}</section><section className="promise-banner"><div><span>PEDEJÁ</span><strong>Uma promessa que se move.</strong><p>Comida, compras, lojas e entregas num só lugar.</p></div><ArrowRight size={20} /></section></div>;
}
