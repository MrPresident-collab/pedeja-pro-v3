...
function Splash() { return <main className="splash"><div className="splash-mark">p</div><div><h1>Pedejá</h1><p>A promessa que se move</p></div></main>; }
function Welcome({ onEnter, onGuest }: { onEnter: () => void; onGuest: () => void }) { return <main className="welcome"><div className="brand-lockup"><span className="brand-dot">p</span><strong>Pedejá</strong><small>A promessa que se move</small></div><div className="welcome-copy"><span>ENTRE NO ECOSSISTEMA</span><h1>Tudo o que precisas,<br />a caminho de ti.</h1><p>Comida, compras, lojas e envios — numa só experiência.</p></div><div className="welcome-actions"><button className="primary" onClick={onEnter}>Entrar</button><button className="secondary" onClick={onEnter}>Criar conta</button><button className="text-button" onClick={onGuest}>Continuar como convidado</button></div></main>; }
