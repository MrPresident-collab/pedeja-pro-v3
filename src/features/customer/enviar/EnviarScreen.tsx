...
const packageTypes: Array<{ value: PackageType; label: string; icon: typeof FileText }> = [
  { value: 'document', label: 'Doc', icon: FileText }, { value: 'keys', label: 'Chaves', icon: KeyRound }, { value: 'shopping', label: 'Compra', icon: ShoppingBag }, { value: 'food', label: 'Comida', icon: Utensils }, { value: 'other', label: 'Outro', icon: Package },
];
const sizeOptions: Array<{ value: PackageSize; label: string; detail: string }> = [
  { value: 'small', label: '0–10 kg', detail: 'Moto' }, { value: 'medium', label: '10–20 kg', detail: 'Moto / Kupapata' }, { value: 'large', label: '20 kg+', detail: 'Car / Carrinha' },
];
const vehicleOptions: Array<{ value: VehicleType; label: string; detail: string; availableFor: PackageSize[] }> = [
  { value: 'moto', label: 'Moto', detail: 'Para volumes até 10 kg', availableFor: ['small', 'medium'] }, { value: 'kupapata', label: 'Kupapata', detail: 'Ideal para volumes de 10–20 kg', availableFor: ['medium', 'large'] }, { value: 'car', label: 'Car / Carrinha', detail: 'Para volumes acima de 20 kg', availableFor: ['large'] },
];

function formatWeight(value: string) { return value ? `${value} kg` : 'Não indicado'; }
...
export default function EnviarScreen({ onBack }: { onBack: () => void }) {
  ...
  const availableVehicles = useMemo(() => form.packageSize ? vehicleOptions.filter(vehicle => vehicle.availableFor.includes(form.packageSize as PackageSize)) : vehicleOptions, [form.packageSize]);
  const recommendedVehicle = useMemo<VehicleType | ''>(() => form.packageSize === 'small' ? 'moto' : form.packageSize === 'medium' ? 'kupapata' : form.packageSize === 'large' ? 'car' : '', [form.packageSize]);
  const canContinue = useMemo(() => {
    if (step === 'route') return form.senderAddress.trim().length >= 2 && form.destinationAddress.trim().length >= 2;
    if (step === 'recipient') return form.recipientMode === 'self' || (form.recipientName.trim().length >= 1 && form.recipientPhone.replace(/\s/g, '').length >= 7);
  ...
  const chooseSize = (value: PackageSize) => {
    ...
    const nextVehicle: VehicleType = value === 'small' ? 'moto' : value === 'medium' ? 'kupapata' : 'car';
    update('vehicleType', nextVehicle);
  };
  const next = () => {
    if (!canContinue) return;
    if (step === 'route') setStep('recipient'); else if (step === 'recipient') setStep('type'); else if (step === 'type') setStep('size'); else if (step === 'size') setStep('fragile'); else if (step === 'fragile') setStep('vehicle'); else if (step === 'vehicle') setStep('prohibited'); else if (step === 'prohibited') setStep('photo'); else if (step === 'photo') setStep('review');
  };
  const back = () => {
    if (mapOpen) { setMapOpen(false); return; }
    if (step === 'route') onBack(); else if (step === 'recipient') setStep('route'); else if (step === 'type') setStep('recipient'); else if (step === 'size') setStep('type'); else if (step === 'fragile') setStep('size'); else if (step === 'vehicle') setStep('fragile'); else if (step === 'prohibited') setStep('vehicle'); else if (step === 'photo') setStep('prohibited'); else setStep('photo');
  };
  const selectCurrentLocation = () => {
    if (!navigator.geolocation) { update('senderAddress', 'Localização atual'); return; }
    navigator.geolocation.getCurrentPosition(() => update('senderAddress', 'Localização atual'), () => update('senderAddress', 'Localização atual'), { enableHighAccuracy: true, timeout: 10000 });
  };
  const handlePhoto = (file: File | null) => { if (!file) return; if (photoPreview) URL.revokeObjectURL(photoPreview); update('packagePhoto', file); setPhotoPreview(URL.createObjectURL(file)); };
  const titleMap: Record<Step, string> = { route: 'De onde para onde?', recipient: 'Quem recebe?', type: 'O que é?', size: 'Tamanho', fragile: 'Frágil?', vehicle: 'Viatura', prohibited: 'Segurança do envio', photo: 'Foto da encomenda', review: 'Resumo do envio' };
  ...
  return (
    <main className="screen enviar-screen">
      <header className="enviar-header"><button className="enviar-back" onClick={back} aria-label="Voltar"><ArrowLeft size={21} /></button><div><span>ENVIAR</span><strong>{stepNumber} de {totalSteps}</strong></div><div className="enviar-header-icon"><Package size={20} /></div></header>
      ...
      <section className="enviar-content">
        ...
        {step === 'route' && <div className="enviar-form route-form"><button className="location-primary" type="button" onClick={selectCurrentLocation}><MapPin size={19} /><span>Usar minha localização atual</span></button><button className="map-secondary" type="button" onClick={() => setMapOpen(true)}><Map size={19} /><span>Escolher no mapa</span><ChevronRight size={17} /></button><div className="recent-block"><div className="section-label">Recentes</div>{recentAddresses.map(address => <button className="recent-row" key={address.label} type="button" onClick={() => update('senderAddress', address.value)}><MapPin size={16} /><span><strong>{address.label}</strong><small>{address.value}</small></span></button>)}</div><div className="enviar-field-card"><div className="field-icon"><MapPin size={19} /></div><label>Destino<input value={form.destinationAddress} onChange={e => update('destinationAddress', e.target.value)} placeholder="Pesquisar morada, bairro ou referência" /><small>O destino pode ser ajustado no mapa.</small></label><Search className="field-end-icon" size={18} /></div></div>}

        {step === 'recipient' && <div className="enviar-form"><div className="choice-grid two"><button type="button" className={`choice-card ${form.recipientMode === 'self' ? 'selected' : ''}`} onClick={() => update('recipientMode', 'self')}><span className="radio-dot" /><strong>Eu mesmo</strong></button><button type="button" className={`choice-card ${form.recipientMode === 'other' ? 'selected' : ''}`} onClick={() => update('recipientMode', 'other')}><span className="radio-dot" /><strong>Outra pessoa</strong></button></div>{form.recipientMode === 'other' && <><div className="enviar-field-card"><div className="field-icon"><Contact size={19} /></div><label>Nome<input value={form.recipientName} onChange={e => update('recipientName', e.target.value)} placeholder="Nome completo" /></label></div><div className="enviar-field-card"><div className="field-icon"><Phone size={19} /></div><label>Número<div className="phone-row"><input value={form.recipientPhone} onChange={e => update('recipientPhone', e.target.value)} placeholder="+244 9xx xxx xxx" inputMode="tel" /><button type="button" aria-label="Importar contacto" title="Importar contacto"><Contact size={18} /></button></div></label></div><div className="enviar-field-card"><div className="field-icon"><Phone size={19} /></div><label>Número alternativo <em>(opcional)</em><input value={form.alternatePhone} onChange={e => update('alternatePhone', e.target.value)} placeholder="+244 9xx xxx xxx" inputMode="tel" /></label></div><label className="plain-field">Nota<textarea value={form.note} onChange={e => update('note', e.target.value)} placeholder="Ex.: Portão azul, falar com a vizinha..." rows={3} /></label></>}</div>}
