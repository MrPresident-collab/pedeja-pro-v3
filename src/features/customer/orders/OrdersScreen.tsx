...
import { ArrowLeft, Clock3, Package, ShoppingBag, Star, Truck, UtensilsCrossed } from 'lucide-react';
...
function formatMoney(value: number, currencyCode: string) {
  ...
  }).format(value) + (currencyCode === 'AOA' ? ' Kz' : ` ${currencyCode}`);
}
...
function statusLabel(status: string) {
  const labels: Record<string, string> = {
    ...
    READY: 'Pronto',
    PICKUP_PENDING: 'A aguardar recolha',
    PICKED_UP: 'Recolhido',
...
function orderKind(order: CustomerOrderHistory): OrderKind {
  ...
  return category.includes('restaurant') || category.includes('food') || category.includes('comida')
    ? 'comida'
    : 'loja';
}

function OrderCard({ order }: { order: CustomerOrderHistory }) {
    ...
    <article className="order-history-card">
      ...
      {delivered && (
        <div className="order-history-delivery">
          <div><Clock3 size={15} /><span>{order.durationMinutes != null ? `${order.durationMinutes} min` : 'Duração indisponível'}</span></div>
          <div><Truck size={15} /><span>{order.riderName ? `Estafeta: ${order.riderName}` : 'Estafeta indisponível'}</span></div>
          {vehicle && <div><span>{vehicle}</span></div>}
        </div>
      )}

      <div className="order-history-footer">
        <span>{order.paymentMethod ? `Pagamento: ${order.paymentMethod}` : 'Pagamento registado'}</span>
        {delivered && <button type="button">{kind === 'comida' ? 'Pedir novamente' : 'Comprar novamente'}</button>}
      </div>
    </article>
  );
}
...
export default function OrdersScreen({ onBack }: { onBack?: () => void }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const data = await getCustomerOrdersHistory();
        if (mounted) setOrders(data);
      } catch (err) {
        console.error('Failed to load customer order history', err);
        if (mounted) setError('Não foi possível carregar os teus pedidos. Tenta novamente.');
      } finally {
  ...
  return <main className="screen orders-screen">
    ...
    <section className="order-list">
      ...
      {!loading && !error && visibleOrders.map(order => <OrderCard key={order.orderId} order={order} />)}
    </section>
