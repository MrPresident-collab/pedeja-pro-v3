CREATE OR REPLACE FUNCTION private.assert_enviar_customer (
  p_shipment_id uuid
)
  RETURNS uuid
  LANGUAGE plpgsql
...
begin
  ...
  select e.customer_id into v_customer_id from public.enviar_shipments e where e.id=p_shipment_id;
...
GRANT EXECUTE ON FUNCTION "private"."assert_enviar_customer"(uuid) TO "postgres";

REVOKE ALL ON FUNCTION "private"."assert_enviar_customer"(uuid) FROM PUBLIC;
